...
public interface EmployeeBusinessRepository extends EntityRepository<Business,String> {
    ...
    @Query("select ea from EmployeeAction ea left join fetch ea.business biz where ea.employee.id = :employeeId and (ea.validTime >= :startDate or false = :hasStartDate) and (ea.validTime <= :endDate or false = :hasEndDate) and (biz.type = :businessType or false = :hasBusinessType) order by ea.validTime desc")
    List<EmployeeAction> findActionData(@QueryParam("employeeId") String employeeId ,@QueryParam("startDate") Date startDate, @QueryParam("hasStartDate") boolean hasStartDate , @QueryParam("endDate") Date endDate, @QueryParam("hasEndDate") boolean hasEndDate,@QueryParam("businessType") Business.Type businessType,@QueryParam("hasBusinessType") boolean hasBusinessType, @FirstResult int firstResult,@MaxResults int maxResults);

    @Query("select count(ea) from EmployeeAction ea where ea.employee.id = :employeeId and (ea.validTime >= :startDate or false = :hasStartDate) and (ea.validTime <= :endDate or false = :hasEndDate) and (ea.business.type = :businessType or false = :hasBusinessType)")
    Long findActionCount(@QueryParam("employeeId") String employeeId ,@QueryParam("startDate") Date startDate, @QueryParam("hasStartDate") boolean hasStartDate , @QueryParam("endDate") Date endDate, @QueryParam("hasEndDate") boolean hasEndDate,@QueryParam("businessType") Business.Type businessType,@QueryParam("hasBusinessType") boolean hasBusinessType);
